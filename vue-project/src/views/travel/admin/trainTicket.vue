    ...
    <div class="card-list">
      <div v-for="(item, idx) in pageData" :key="idx" class="train-card">
        <el-card shadow="hover">
          ...
          <el-form :model="editIndex === idx ? editForm : item" label-width="110px" size="default" style="margin-top:10px">
            <div class="grid-form">
              <el-form-item label="车次">
                <el-input v-model="(editIndex===idx?editForm:item).trainNo" :disabled="editIndex!==idx" />
              </el-form-item>
              <el-form-item label="列车类型">
                <el-input v-model="(editIndex===idx?editForm:item).trainType" :disabled="editIndex!==idx" />
              </el-form-item>
              <el-form-item label="车型">
                <el-input v-model="(editIndex===idx?editForm:item).trainModel" :disabled="editIndex!==idx" />
              </el-form-item>
              ...
              <el-form-item label="发站">
                <el-autocomplete
                    ...
                    @select="(item)=>{if(editIndex===idx)editForm.startStation=item.value}"
                    @blur="(e)=>handleBlurField(e, editIndex===idx ? editForm : null, 'startStation')"
                />
              </el-form-item>
              <el-form-item label="到站">
                <el-autocomplete
                    ...
                    placeholder="到站"
                    :disabled="editIndex!==idx"
                    @select="(item)=>{if(editIndex===idx)editForm.endStation=item.value}"
                    @blur="(e)=>handleBlurField(e, editIndex===idx ? editForm : null, 'endStation')"
                />
              </el-form-item>
              <el-form-item label="始发站">
                <el-autocomplete
                    ...
                    placeholder="始发站"
                    :disabled="editIndex!==idx"
                    @select="(item)=>{if(editIndex===idx)editForm.originStation=item.value}"
                    @blur="(e)=>handleBlurField(e, editIndex===idx ? editForm : null, 'originStation')"
                />
              </el-form-item>
              <el-form-item label="终到站">
                <el-autocomplete
                    ...
                    placeholder="终到站"
                    :disabled="editIndex!==idx"
                    @select="(item)=>{if(editIndex===idx)editForm.terminalStation=item.value}"
                    @blur="(e)=>handleBlurField(e, editIndex===idx ? editForm : null, 'terminalStation')"
                />
              </el-form-item>

              <el-form-item label="发车时间">
                <el-date-picker v-model="(editIndex===idx?editForm:item).departureDatetime"
                                type="datetime" style="width:100%" :disabled="editIndex!==idx" />
              </el-form-item>
              <el-form-item label="到达时间">
                <el-date-picker v-model="(editIndex===idx?editForm:item).arrivalDatetime"
                                type="datetime" style="width:100%" :disabled="editIndex!==idx" />
              </el-form-item>
              <el-form-item label="座位号">
                <el-input v-model="(editIndex===idx?editForm:item).seatNo" :disabled="editIndex!==idx" />
              </el-form-item>
              <el-form-item label="座位等级">
                <el-select v-model="(editIndex===idx?editForm:item).seatClass"
                           style="width:100%" :disabled="editIndex!==idx">
              ...
              <el-form-item label="里程(km)">
                <el-input v-model.number="(editIndex===idx?editForm:item).mileageKm" :disabled="editIndex!==idx" />
              </el-form-item>
            </div>
            ...
            <div class="station-section">
              <div class="station-header" @click="toggleStation(idx)">
                <span>途经站 {{ item.stations?.length || 0 }} 个</span>
                <span>{{ expandIdx === idx ? '收起 ▲' : '展开 ▼' }}</span>
              </div>
              <el-timeline v-if="expandIdx === idx" style="margin-top:10px">
                <el-timeline-item v-for="(st, sidx) in (editIndex===idx?editForm.stations:item.stations)" :key="sidx"
                                  :timestamp="st.time">
                  {{ st.name }}
                </el-timeline-item>
              </el-timeline>
...
const trainList = ref([
  {
    trainNo: 'G123', trainType: '高铁', trainModel: 'CR400AF',
    startStation: '北京南', endStation: '上海虹桥', originStation: '北京南', terminalStation: '上海虹桥',
    departureDatetime: '2026-04-10 08:00:00', arrivalDatetime: '2026-04-10 12:32:00',
    seatNo: '05A', seatClass: '二等座', mileageKm: 1318,
    stations: [
      { name: '北京南', time: '08:00' },{ name: '天津南', time: '09:12' },
      { name: '济南西', time: '10:30' },{ name: '徐州东', time: '11:45' },{ name: '上海虹桥', time: '12:32' }
    ]
  }
])
